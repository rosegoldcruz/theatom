...
export function ATOMSidebar({ className = '' }: ATOMSidebarProps) {
  ...
  // UPDATE: Use URLs instead of page IDs
  const navigation = [
    { href: '/dashboard', icon: Home, label: 'Dashboard' },
    { href: '/dashboard/bot', icon: Bot, label: 'Bot Control' },
    { href: '/dashboard/opportunities', icon: TrendingUp, label: 'Opportunities' },
  ...
  return (
    <div className={`w-64 ${isDark ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'} border-r transition-all duration-300 ${className}`}>
      <div className="flex flex-col h-full">
        ...
        {/* Navigation - CHANGED: buttons to Links, same styling */}
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <button
            type="button"
            className={`w-full flex items-center space-x-2 px-3 py-2 text-sm ${isDark ? 'text-gray-400 hover:text-gray-300' : 'text-gray-500 hover:text-gray-700'} rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors`}
          >
